...
// src/options.js
var SUPABASE_URL = "https://hqcklghtagnlihrccyqo.supabase.co";
var SUPABASE_ANON_KEY = "sb_publishable_62VlwOdM2EdbgEjyndfD1g_dW2WvWm5";
var WEB_APP_ORIGIN = "http://localhost:3000";
...
async function updateUI() {
  ...
  if (session) {
    ...
    els.recordSection.style.display = "none";
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signInWithOAuth({
        ...
        options: {
          redirectTo,
          skipBrowserRedirect: true
          // distinct from 'redirectTo' usage in some versions, but strictly we need the URL
        }
      });
      if (error)
        throw error;
...
async function handleUploadFlow(blob) {
  try {
    ...
    if (meetingError)
      throw new Error("Create Meeting: " + meetingError.message);
    updateStatus("Uploading video...");
    const fileName = `${artifactId}.webm`;
    const storagePath = `${user.id}/${meetingId}/${fileName}`;
    const { error: uploadError } = await supabase.storage.from("meeting-media").upload(storagePath, blob, {
    ...
    if (artifactError)
      throw new Error("Create Artifact: " + artifactError.message);
